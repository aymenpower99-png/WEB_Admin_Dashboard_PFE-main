...
import UpcomingRidesPage  from "./Dashboard/UpcomingRides";
import PastRidesPage      from "./Dashboard/PastRides";
...
function ProfilePage() {
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:"1.5rem", maxWidth:600 }}>
      <h1 className="ts-page-title" style={{ fontSize:"1.25rem", fontWeight:800 }}>My Profile</h1>
      ...
      <div style={{
        ...
        <div>
          <div style={{ fontSize:"1.125rem", fontWeight:700, color:"var(--text-h)" }}>Sarah Lee</div>
          <div style={{ fontSize:"0.8125rem", color:"var(--text-muted)", marginTop:2 }}>Agency Manager</div>
          <div style={{
      ...
      <div style={{
        ...
        <div className="ts-section-label" style={{ marginBottom:"0.25rem" }}>Account Information</div>
        {[
          { label:"Full Name",    value:"Sarah Lee" },
          { label:"Email",        value:"sarah@moviroo.com" },
          { label:"Phone",        value:"+44 7700 900 123" },
          { label:"Role",         value:"Agency Manager" },
          { label:"Member Since", value:"January 2023" },
        ].map(({ label, value }) => (
          <div key={label} style={{
            display:"flex", justifyContent:"space-between", alignItems:"center",
            paddingBottom:"0.85rem", borderBottom:"1px solid var(--border)",
          }}>
            <span style={{ fontSize:"0.8125rem", color:"var(--text-muted)", fontWeight:500 }}>{label}</span>
            <span style={{ fontSize:"0.8125rem", color:"var(--text-h)", fontWeight:600 }}>{value}</span>
          </div>
        ))}
      </div>
    </div>
...
function Shell({
  ...
  const navigate = (page: string, prefill?: Driver | Vehicle | null) => {
    ...
    if (page === "agency-drivers")  setEditDriver(prefill !== undefined ? (prefill as Driver | null)  : null);
    if (page === "agency-vehicles") setEditVehicle(prefill !== undefined ? (prefill as Vehicle | null) : null);
  ...
  return (
    <div
      className={dark ? "dark" : ""}
      style={{ display:"flex", height:"100vh", overflow:"hidden", background:"var(--bg-page)", fontFamily:"var(--font)" }}
    >
      {/* ── Sidebar ── */}
      {sidebarOpen && (
        <aside style={{
