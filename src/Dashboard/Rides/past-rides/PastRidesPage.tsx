...
import { ridesApi, filterPast, passengerName, driverName, fmtDate, fmtTime } from "../../../api/rides";

import { ROWS, ROW_H, TH, TD } from "../shared/RideTypes";
import { RideStatusBadge } from "../shared/RideStatusBadge";
import { ActionButton, IconEye, HOVER } from "../shared/RideActionButtons";
    ...
    <>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "0.85rem" }}>
        ...
        <div className="ts-page-header">
          <div><h1 className="ts-page-title">Past Rides</h1></div>
        </div>
                    ...
                    <>
                      {paged.map(ride => (
                        <tr key={ride.id} className="ts-tr" style={{ height: ROW_H }}>
                          ...
                          <td style={TD}>
                            <div style={{ fontWeight: 600, color: "var(--text-h)" }}>{fmtDate(ride.scheduledAt)}</div>
                            <div style={{ fontSize: ".75rem", color: "var(--text-muted)" }}>{fmtTime(ride.scheduledAt)}</div>
                          </td>
