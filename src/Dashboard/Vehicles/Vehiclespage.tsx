  ...
  function loadVehicles() {
    ...
    apiClient
      ...
      .then(async (res) => {
        ...
        const driverIds = [
          ...new Set(list.map((v: any) => v.driverId).filter(Boolean)),
        ] as string[];

        if (driverIds.length > 0) {
          try {
            const drRes = await apiClient.get("/drivers", {
              params: { limit: 200 },
            });
            const drRaw = drRes.data;
            const drList: any[] = Array.isArray(drRaw)
              ? drRaw
              : (drRaw?.data ?? []);
          ...
          {TABS.map((t) => (
            <button
              ...
              style={{
                ...
                border: "none",
                background:
                  activeFilter === t.key ? "#7c3aed" : "var(--bg-inner)",
                ...
                {filtered.length === 0 ? (
                  ...
                  paged.map((v) => (
                    <VehicleTableRow
                      ...
                      v={v}
                      onEdit={(vehicle) =>
                        onNavigate("agency-vehicles", vehicle)
                      }
