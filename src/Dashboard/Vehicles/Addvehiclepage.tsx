...
import {
  ...
  PlainDropdown,
  VehicleClassGrid,
} from "./AddvehicleComponents/Field";
import {
  MakeAutocomplete,
  ModelAutocomplete,
} from "./AddvehicleComponents/VehicleAutocompletes";
  ...
  const [form, setForm] = useState<FormState>(
    prefill
      ? {
          ...
          color: COLORS.includes(prefill.color as any)
            ? (prefill.color ?? "")
            : "",
    ...
    try {
      if (isEdit) {
        ...
        if (
          ...
          form.vehicleClass &&
          classMap.has(form.vehicleClass)
        ) {
        ...
        const updated = mapBackendVehicle(rawData, new Map());
