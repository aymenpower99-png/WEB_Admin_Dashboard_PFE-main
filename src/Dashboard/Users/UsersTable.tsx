...
export default function UsersTable({ dark: _ }: UsersTableProps) {
  ...
  const rows: (AdminUser | null)[] = [
    ...paginated,
    ...Array<null>(ROWS_PER_PAGE - paginated.length).fill(null),
  ];
  ...
  return (
    <div className="ts-card overflow-hidden" style={{ padding: "1.5rem" }}>
      ...
      <div className="grid text-xs font-medium pb-2 border-b ts-faint" style={{ gridTemplateColumns: GRID, ...BORDER }}>
        <span>Name</span>
        <span>Email</span>
        <span>Role</span>
        <span>Status</span>
      ...
      {loading ? (
        <div style={{ padding: "2rem", textAlign: "center", color: "var(--text-faint)", fontSize: ".8rem" }}>
          Loading users…
        </div>
      ) : (
            ...
            >
              {user ? (
                <>
                  ...
                  <span>
                    <span className={STATUS_PILL[user.status] ?? "ts-pill"}>
                      {user.status.charAt(0).toUpperCase() + user.status.slice(1)}
                    </span>
                  </span>
                  <span className="ts-td-h font-medium">{user.trips ?? 0}</span>
                </>
              ) : (
                <>
                  <span>&nbsp;</span>
      ...
      <div className="flex items-center justify-between mt-4 pt-3 border-t ts-faint text-xs" style={BORDER}>
        <span>
          Showing {users.length === 0 ? 0 : (page - 1) * ROWS_PER_PAGE + 1}–
          {Math.min(page * ROWS_PER_PAGE, users.length)} of {total || users.length} entries
        </span>
        <div className="flex items-center gap-1">
          <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1} className="px-2 py-1 rounded ts-link disabled:opacity-30 disabled:cursor-not-allowed">
            Previous
          </button>
          {Array.from({ length: totalPages }, (_, i) => i + 1).map((n) => (
            <button key={n} onClick={() => setPage(n)} className="w-6 h-6 rounded text-xs font-medium flex items-center justify-center" style={{ background: n === page ? "var(--accent)" : "transparent", color: n === page ? "#fff" : "inherit" }}>
              {n}
            </button>
          ))}
          <button onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={page === totalPages} className="px-2 py-1 rounded ts-link disabled:opacity-30 disabled:cursor-not-allowed">
            Next
          </button>
        </div>
      </div>
