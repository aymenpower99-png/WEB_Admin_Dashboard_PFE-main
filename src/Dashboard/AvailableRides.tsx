...
const ROW_H = 96;   // fixed row height — generous vertical breathing room

/* ─── MOCK DATA ──────────────────────────────────────────────────────── */
const MOCK_RIDES: RideRequest[] = [
  { id: 1,  rider: "Sara M",    pickup: "Tunis-Carthage Airport",  drop: "City Center, Tunis",     vehicleType: "Sedan",   date: "Today",    time: "10:30",  fare: 28,  distance: "12 km",  duration: "22 min",  status: "waiting", passengerCount: 2 },
  { id: 2,  rider: "Omar S",    pickup: "El Mechtel Hotel",        drop: "Tunis Train Station",    vehicleType: "SUV",     date: "Today",    time: "11:15",  fare: 45,  distance: "8 km",   duration: "18 min",  status: "waiting", passengerCount: 4 },
  { id: 3,  rider: "Lina B",    pickup: "La Marsa Beach",          drop: "Bardo Museum",           vehicleType: "Sedan",   date: "Today",    time: "12:00",  fare: 35,  distance: "18 km",  duration: "30 min",  status: "waiting", passengerCount: 1 },
  { id: 4,  rider: "Karim T",   pickup: "Lac Business District",   drop: "Hammamet Resort",        vehicleType: "Van",     date: "Today",    time: "13:45",  fare: 120, distance: "65 km",  duration: "55 min",  status: "waiting", passengerCount: 6 },
  { id: 5,  rider: "Nadia R",   pickup: "Sfax City Center",        drop: "Sfax Airport",           vehicleType: "Sedan",   date: "Today",    time: "14:00",  fare: 22,  distance: "9 km",   duration: "15 min",  status: "waiting", passengerCount: 1 },
  { id: 6,  rider: "Youssef A", pickup: "Sousse Medina",           drop: "Port El Kantaoui",       vehicleType: "SUV",     date: "Tomorrow", time: "09:00",  fare: 55,  distance: "14 km",  duration: "20 min",  status: "waiting", passengerCount: 3 },
  { id: 7,  rider: "Fatima Z",  pickup: "Nabeul Bus Station",      drop: "Hammamet North",         vehicleType: "Sedan",   date: "Tomorrow", time: "10:30",  fare: 18,  distance: "7 km",   duration: "12 min",  status: "waiting", passengerCount: 1 },
];
...
const VEHICLE_ICONS: Record<string, React.ReactNode> = {
  Sedan: <DirectionsCarRoundedIcon style={{ fontSize: 14 }} />,
  SUV:   <DirectionsCarRoundedIcon style={{ fontSize: 14 }} />,
  Van:   <TwoWheelerRoundedIcon    style={{ fontSize: 14 }} />,
};

/* ─── PAGINATION (matches DriversPage exactly) ───────────────────────── */
    ...
    <div className="ts-overlay" onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="ts-modal ts-modal-scroll" style={{ maxWidth: "28rem" }}>
        <div className="ts-modal-header">
          <div>
            <p style={{ fontWeight: 700, fontSize: ".9rem", color: "var(--text-h)" }}>Assign Driver</p>
            <p style={{ fontSize: ".72rem", color: "var(--text-muted)", marginTop: ".1rem" }}>
              For {ride.rider} · {ride.pickup} → {ride.drop}
            </p>
          </div>
          ...
          {filtered.map(driver => (
            ...
            >
              <div style={{
                ...
                background: selected === driver.id ? "#ede9fe" : "#e9d5ff",
                display: "flex", alignItems: "center", justifyContent: "center",
                fontWeight: 700, fontSize: ".72rem",
                color: selected === driver.id ? "#6d28d9" : "#7c3aed",
              }}>
                {driver.avatar}
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: "flex", alignItems: "center", gap: ".4rem" }}>
                  ...
                  <span style={{
                    fontSize: ".62rem", fontWeight: 600, padding: ".1rem .4rem", borderRadius: "9999px",
                    background: driver.status === "available" ? "var(--active-bg)" : "#fef3c7",
                    color:      driver.status === "available" ? "var(--active-fg)" : "#92400e",
                  }}>
        ...
        <div className="ts-modal-footer">
          ...
          <button
            ...
            onClick={() => {
              ...
              onAssign(ride.id, driver);
              onClose();
            }}
          >
            <CheckRoundedIcon style={{ fontSize: 14 }} /> Confirm Assignment
          </button>
        </div>
      </div>
    </div>
...
/* ─── RIDE DETAILS MODAL ─────────────────────────────────────────────── */
function RideDetailsModal({ ride, onClose }: { ride: RideRequest; onClose: () => void }) {
  return (
    <div className="ts-overlay" onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="ts-modal">
        <div className="ts-modal-header">
          <div>
            ...
            <p style={{ fontSize: ".72rem", color: "var(--text-muted)", marginTop: ".1rem" }}>Request #{ride.id}</p>
          </div>
          <button className="ts-modal-close" onClick={onClose}>
            <CloseRoundedIcon style={{ fontSize: 16 }} />
          </button>
        ...
        <div className="ts-modal-body">
          <div className="ts-card-inner" style={{ padding: ".875rem 1rem", display: "flex", alignItems: "center", gap: ".75rem" }}>
            ...
            <span className="ts-pill ts-pill-pending" style={{ marginLeft: "auto" }}>Waiting</span>
          </div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: ".5rem" }}>
            <DirectionsCarRoundedIcon style={{ fontSize: 16, color: "var(--text-muted)" }} />
            <span style={{ fontSize: ".8rem", color: "var(--text-body)" }}>Requested: <strong>{ride.vehicleType}</strong></span>
          </div>
        </div>
        <div className="ts-modal-footer">
...
/* ─── SHARED STYLES (matches DriversPage) ────────────────────────────── */
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1rem", width: "100%" }}>
      ...
      <div className="ts-page-header">
        ...
        <div style={{ display: "flex", alignItems: "center", gap: ".5rem" }}>
          <span style={{
            padding: ".25rem .75rem", borderRadius: "9999px", fontSize: ".72rem", fontWeight: 700,
      ...
      <div className="ts-filter-bar">
        <div className="ts-search-bar" style={{ flex: 1, maxWidth: 280 }}>
          <SearchRoundedIcon style={{ fontSize: 14 }} />
          <input
            placeholder="Search rider, pickup, destination…"
            value={search}
        ...
        <div style={{ display: "flex", gap: ".25rem" }}>
          {TYPES.map(t => (
            <button
              key={t}
              className={`ts-filter-chip${filterType === t ? " ts-active" : ""}`}
      ...
      <div className="ts-table-wrap" style={{ display: "flex", flexDirection: "column" }}>
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", tableLayout: "fixed" }}>
            ...
            {/* ── thead with bold uppercase headers matching DriversPage ── */}
            <thead>
              <tr>
                {["Rider", "Route", "Vehicle", "Schedule", "Fare / Distance", "Actions"].map(h => (
            ...
            <tbody>
              {paged.length === 0 ? (
                <>
                  <tr style={{ height: ROW_H }}>
                    <td colSpan={6} style={{ ...TD, textAlign: "center", color: "var(--text-faint)" }}>
                      No ride requests match your filters.
                    </td>
                  </tr>
                ...
                <>
                    ...
                    <tr key={ride.id} className="ts-tr" style={{ height: ROW_H }}>
                      ...
                      <td style={TD}>
                        <div style={{ display: "flex", alignItems: "center", gap: ".6rem" }}>
                          <div style={{
                            ...
                            justifyContent: "center", fontWeight: 700, fontSize: ".68rem", color: "#6d28d9",
                          }}>
                            {ride.rider.slice(0, 2).toUpperCase()}
                          </div>
                          <div>
                      ...
                      <td style={TD}>
                        <div style={{ display: "inline-flex", alignItems: "center", gap: ".35rem", padding: ".25rem .6rem", borderRadius: "9999px", background: "var(--bg-inner)", border: "1px solid var(--border)" }}>
                          ...
                          <span style={{ fontSize: ".75rem", fontWeight: 500, color: "var(--text-body)" }}>{ride.vehicleType}</span>
                        </div>
                      </td>
                      ...
                      <td style={TD}>
                        ...
                        <span className="ts-pill ts-pill-pending" style={{ marginTop: ".3rem" }}>Waiting</span>
                      </td>
                  ...
                  {/* Ghost rows — keep table height fixed across all pages */}
        ...
        {/* ── Pagination (identical to DriversPage) ── */}
        <Pagination
          page={safePage}
      ...
      {toast && (
        <div style={{
          ...
          padding: ".625rem 1rem", borderRadius: ".6rem",
          background: "#111827", color: "#fff", fontSize: ".78rem", fontWeight: 500,
          display: "flex", alignItems: "center", gap: ".5rem",
          boxShadow: "0 4px 20px rgba(0,0,0,.25)",
          animation: "tsSettingsIn .25s ease",
        }}>
          <CheckRoundedIcon style={{ fontSize: 14, color: "#34d399" }} />
          {toast}
        </div>
      )}
    </div>
