...
export default function ConversationTab({
  ...
  return (
    <div className="flex flex-col h-full">
        ...
        {messages.map((msg) => {
          ...
          return (
            <div key={msg.id} className={`flex flex-col ${isAdmin ? "items-end" : "items-start"} group`}>
              ...
              {isEditing ? (
                <div className={`max-w-[70%] w-full px-4 py-3 rounded-2xl text-sm leading-relaxed border ${
                  dark ? "bg-gray-800 border-gray-700" : "bg-white border-gray-200"
                }`}>
                  <textarea
                    value={editValue}
      ...
      {isResolved ? (
        <div className={`flex-shrink-0 border-t px-4 py-3 text-center ${dark ? "border-gray-800 bg-gray-900" : "border-gray-200 bg-white"}`}>
          <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium ${dark ? "bg-gray-800 text-gray-400" : "bg-gray-100 text-gray-500"}`}>
