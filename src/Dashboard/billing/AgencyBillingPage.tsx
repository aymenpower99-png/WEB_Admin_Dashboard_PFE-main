...
export default function AgencyBillingPage() {
  ...
  const summaryCards = [
    { label: "Total Revenue (This Month)", value: stats ? `${stats.totalEarnings.toLocaleString()} TND` : "—", icon: <TrendingUpRoundedIcon style={{ fontSize: 18 }} />, accent: "#10b981" },
    { label: "Paid Revenue",               value: stats ? `${stats.paidRevenue.toLocaleString()} TND` : "—", icon: <MonetizationOnRoundedIcon style={{ fontSize: 18 }} />, accent: "#3b82f6" },
    { label: "Pending Payments",           value: stats ? `${stats.pendingPayments.toLocaleString()} TND` : "—", icon: <PendingActionsRoundedIcon style={{ fontSize: 18 }} />, accent: "#f59e0b" },
    { label: "Total Trips",               value: stats ? `${stats.totalTrips}` : "—", icon: <AccountBalanceWalletRoundedIcon style={{ fontSize: 18 }} />, accent: "#8b5cf6" },
  ];
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="ts-page-header">
        ...
        <button className="ts-btn-ghost" style={{ padding: "0.3rem 0.85rem", fontSize: "0.72rem", display: "flex", alignItems: "center", gap: "0.3rem" }}
          onClick={fetchDashboard} disabled={loadingStats}>
