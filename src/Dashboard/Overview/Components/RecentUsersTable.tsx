...
import { RoleBadge, StatusBadge } from "../Badge_action_buttons/StatusBadges";
...
export default function RecentUsersTable({ users, dark }: RecentUsersTableProps) {
  const [page, setPage] = useState(1);
  const totalPages = Math.max(1, Math.ceil(users.length / USERS_PER_PAGE));
  const paged = users.slice((page - 1) * USERS_PER_PAGE, page * USERS_PER_PAGE);
            ...
            paged.map((u) => (
              <tr key={u.id} style={{ height: 88 }}>
                <td style={TD}>
                  ...
                  <span style={{ fontSize: ".78rem", color: "var(--text-muted)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", display: "block" }}>
                    {u.email}
                  </span>
                </td>
                <td style={TD}><RoleBadge role={u.role} /></td>
                <td style={TD}><StatusBadge status={u.status} /></td>
                <td style={{ ...TD, color: u.trips === null ? "var(--text-faint)" : "var(--text-body)" }}>
                  {u.trips ?? "—"}
                </td>
              </tr>
            ))
