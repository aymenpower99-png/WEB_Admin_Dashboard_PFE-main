...
import type { RevenueEntry } from ".";
import { fetchRevenueData } from "./mockData";
    ...
    <div className="rounded-xl border p-4" style={{ background: surface, borderColor: border }}>
      <div className="flex items-start justify-between mb-4">
        <div>
          ...
          <p style={{ fontSize: 11, color: sub, marginTop: 1 }}>Daily gross revenue across all sectors</p>
        </div>
        <select
          value={days}
        ...
        <ResponsiveContainer width="100%" height={160}>
          <AreaChart data={revenueData} margin={{ top: 5, right: 5, bottom: 0, left: -20 }}>
            ...
            <Area type="monotone" dataKey="revenue" name="Revenue"
              stroke={C.primaryPurple} strokeWidth={2.5} fill="url(#revGrad)"
              dot={{ fill: C.primaryPurple, r: 3 }} activeDot={{ r: 5, strokeWidth: 0 }} />
          </AreaChart>
        </ResponsiveContainer>
      )}
    </div>
