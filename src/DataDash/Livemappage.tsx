...
// Convert backend DriverLocation to frontend Driver
function convertToDriver(loc: DriverLocation): Driver {
  ...
  return {
    ...
    lng: loc.longitude,
    speed: loc.speed_kmh,
    battery: 80, // Backend doesn't provide battery, use default
    rating: loc.rating,
...
const DEFAULT_CENTER: [number, number] = [10.18, 36.82];

// No coordinate conversion needed - use real lat/lng from backend
function toRealCoords(lat: number, lng: number): [number, number] {
  return [lng, lat];
}
...
const MAP_EVENTS: MapEvent[] = []; // Empty - remove static surge zones
...
export function LiveMapPage({ dark }: { dark: boolean }) {
  ...
  useEffect(() => {
    const fetchDrivers = async () => {
      try {
        const data = await liveMapApi.getOnlineDrivers(); // Remove online_only filter to show all drivers
        const converted = data.map(convertToDriver);
  ...
  useEffect(() => {
    ...
    try {
      ...
      socket.on("connect", () => {
        console.log("WebSocket connected to live map");
      ...
      socket.on("admin:driver_location_update", (data: any) => {
        setDrivers((prev) => {
          ...
          if (existingIndex >= 0) {
            ...
            const updated = [...prev];
            updated[existingIndex] = convertToDriver({
              driver_id: data.driver_id,
              latitude: data.latitude,
              longitude: data.longitude,
              heading: 0, // Backend doesn't send heading in update
              speed_kmh: data.speed_kmh,
              is_online: data.is_online,
              last_seen_at: data.last_seen_at,
              progress: data.progress,
              rating: prev[existingIndex].rating,
              total_trips: prev[existingIndex].trips,
              name: prev[existingIndex].name,
            });
            return updated;
          } else {
            // Add new driver
            return [
              ...
              convertToDriver({
                ...
                rating: 4.5,
                total_trips: 0,
                name: `Driver ${data.driver_id.slice(0, 8)}`,
              }),
            ];
          }
        });
      ...
      const refreshInterval = setInterval(async () => {
        try {
          const data = await liveMapApi.getOnlineDrivers(); // Show all drivers including offline
          const converted = data.map(convertToDriver);
          setDrivers(converted);
          setTick((t) => t + 1);
        } catch (error) {
      ...
      // Continue with periodic polling only
      const refreshInterval = setInterval(async () => {
        try {
          const data = await liveMapApi.getOnlineDrivers(); // Show all drivers including offline
          const converted = data.map(convertToDriver);
      ...
      >
        ...
        >
          ...
          >
            ...
            <p style={{ fontSize: 10, color: "rgba(255,255,255,.4)" }}>
